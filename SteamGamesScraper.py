...
def extract_app_ids_from_file(file_path, output_file_path):
    # Read the file contents
    with open(file_path, 'r') as file:
        content = file.read()

    # Use regular expression to extract app IDs
    pattern = r'data-ds-appid="(\d+)"'
    app_ids = re.findall(pattern, content)

    # Write the app IDs to a JSON file
    with open(output_file_path, 'w') as output_file:
        json.dump(app_ids, output_file)
...
def SteamRequest(appID, retryTime, successRequestCount, errorRequestCount, retries, currency=DEFAULT_CURRENCY,
                 language='en'):
    '''
    Request and parse information about a Steam app.
    '''
    url = "http://store.steampowered.com/api/appdetails/"
    response = DoRequest(url, {"appids": appID, "cc": currency, "l": language}, retryTime, successRequestCount,
                         errorRequestCount, retries)
    if response:
        try:
            data = response.json()
            app = data[appID]
            if app['success'] == False:
                ...
                return None
            elif app['data']['is_free'] == False and 'price_overview' in app['data'] and app['data']['price_overview'][
                'final_formatted'] == '':
                return None
...
def ParseSteamGame(app):
    ...
    game['release_date'] = app['release_date']['date'] if 'release_date' in app and not app['release_date'][
        'coming_soon'] else ''
    ...
    game['notes'] = app['content_descriptors']['notes'] if 'content_descriptors' in app and app['content_descriptors'][
        'notes'] is not None else ''
    ...
    if 'package_groups' in app:
        for package in app['package_groups']:
            ...
            game['packages'].append(
                {'title': SanitizeText(package['title']), 'description': SanitizeText(package['description']),
                 'subs': subs})
...
def Scraper(dataset, notreleased, discarted, args):
    ...
    if apps:
            ...
            if appID not in dataset and appID not in discarted:
                ...
                if app:
                    ...
                    if game['release_date'] != '':
                        if args.steamspy:
                            extra = SteamSpyRequest(appID, min(4, args.sleep), successRequestCount, errorRequestCount,
                                                    args.retries)
                            if extra != None:
        ...
        print('\r')
        Log(INFO,
            f'Scanning completed: {gamesAdded} new games added, {gamesNotReleased} not released, {gamesDiscarted} discarted')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--autosave', type=int, default=DEFAULT_AUTOSAVE,
